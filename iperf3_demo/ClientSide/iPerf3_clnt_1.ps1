...
netsh interface ip show config $ifName | Select-String -Pattern "Default Gateway:" > .\dgwIpAddr1.txt
$temp = Get-Content -Filter "Default Gateway:" .\dgwIpAddr1.txt
...
IF ($result2 -eq "True") {
    ...
    Write-Host "Start Upstream"
    C:\"Program Files"\iperf3\iperf3.exe -c $svrIpAddr --port $targetPort1 --interval $interval --time $time --parallel $parallel 
}
else {
